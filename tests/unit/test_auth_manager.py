...
class TestKiroAuthManagerSsoRegionSeparation:
    ...
    def test_api_region_uses_sso_region_as_fallback_when_no_profile_arn(
        self, temp_sqlite_db
    ):
        """
        ...
        """
        print(
            f"Setup: Creating KiroAuthManager with SQLite (SSO region=eu-west-1, no profile ARN)..."
        )
        manager = KiroAuthManager(sqlite_db=temp_sqlite_db)
        ...
        print(
            f"Comparing _detected_api_region: Expected None, Got '{manager._detected_api_region}'"
        )
...
class TestKiroAuthManagerSqliteWriteBackPreservation:
    ...
    def test_save_preserves_unknown_fields_social_login(self, tmp_path):
        ...
        print("Verification: Tokens updated...")
        print(
            f"Comparing access_token: Expected 'new_social_access', Got '{saved_data['access_token']}'"
        )
        assert saved_data["access_token"] == "new_social_access"

        print(
            f"Comparing refresh_token: Expected 'new_social_refresh', Got '{saved_data['refresh_token']}'"
        )
        assert saved_data["refresh_token"] == "new_social_refresh"
        ...
        print(
            f"Comparing provider: Expected 'google', Got '{saved_data.get('provider')}'"
        )
        ...
        print(
            f"Comparing customField: Expected 'custom_value', Got '{saved_data.get('customField')}'"
        )
    ...
    def test_save_preserves_unknown_fields_oidc(self, tmp_path):
        ...
        print(
            f"Comparing unknownField1: Expected 'value1', Got '{saved_data.get('unknownField1')}'"
        )
        assert saved_data.get("unknownField1") == "value1"

        print(
            f"Comparing unknownField2: Expected 'value2', Got '{saved_data.get('unknownField2')}'"
        )
    ...
    def test_save_updates_only_our_fields(self, tmp_path):
        ...
        print(
            f"Comparing access_token: Expected 'new_token', Got '{saved_data['access_token']}'"
        )
        assert saved_data["access_token"] == "new_token"

        print(
            f"Comparing refresh_token: Expected 'new_refresh', Got '{saved_data['refresh_token']}'"
        )
        assert saved_data["refresh_token"] == "new_refresh"

        print(
            f"Comparing region: Expected 'eu-central-1', Got '{saved_data['region']}'"
        )
        assert saved_data["region"] == "eu-central-1"

        print("Verification: Custom field unchanged...")
        print(
            f"Comparing customField: Expected 'original_value', Got '{saved_data.get('customField')}'"
        )
    ...
    def test_save_with_sqlite_readonly_flag(self, tmp_path, monkeypatch):
        ...
        print(
            f"Comparing access_token: Expected 'original_access', Got '{saved_data['access_token']}'"
        )
        assert saved_data["access_token"] == "original_access"

        print(
            f"Comparing refresh_token: Expected 'original_refresh', Got '{saved_data['refresh_token']}'"
        )
    ...
    def test_save_fallback_when_primary_key_deleted(self, tmp_path):
        ...
        print(
            f"Comparing access_token: Expected 'fallback_access', Got '{saved_data['access_token']}'"
        )
        assert saved_data["access_token"] == "fallback_access"

        print(
            f"Comparing refresh_token: Expected 'fallback_refresh', Got '{saved_data['refresh_token']}'"
        )
    ...
    def test_save_fallback_when_primary_key_is_none(self, tmp_path):
        ...
        print(
            f"Comparing access_token: Expected 'none_key_access', Got '{saved_data['access_token']}'"
        )
        assert saved_data["access_token"] == "none_key_access"

        print(
            f"Comparing refresh_token: Expected 'none_key_refresh', Got '{saved_data['refresh_token']}'"
        )
    ...
    def test_save_handles_corrupted_json_gracefully(self, tmp_path):
        ...
        print(
            "Verification: Tokens saved to fallback key (skipped corrupted primary)..."
        )
        conn = sqlite3.connect(str(db_file))
        cursor = conn.cursor()
        cursor.execute(
        ...
        print(
            f"Comparing access_token: Expected 'corrupted_test_access', Got '{saved_data['access_token']}'"
        )
    ...
    @pytest.mark.asyncio
    async def test_save_after_token_refresh_kiro_desktop(
        self, tmp_path, mock_kiro_token_response
    ):
        """
        ...
        with patch("kiro.auth.httpx.AsyncClient") as mock_client_class:
            ...
            print(
                f"Comparing refresh_token: Expected 'new_refresh_token_xyz', Got '{saved_data['refresh_token']}'"
            )
            assert saved_data["refresh_token"] == "new_refresh_token_xyz"
            ...
            print(
                f"Comparing provider: Expected 'github', Got '{saved_data.get('provider')}'"
            )
            assert saved_data.get("provider") == "github"

            print(
                f"Comparing customField: Expected 'must_be_preserved', Got '{saved_data.get('customField')}'"
            )
            assert saved_data.get("customField") == "must_be_preserved"

    @pytest.mark.asyncio
    async def test_save_after_token_refresh_aws_sso_oidc(
        self, tmp_path, mock_aws_sso_oidc_token_response
    ):
        """
        ...
        with patch("kiro.auth.httpx.AsyncClient") as mock_client_class:
            ...
            print("Verification: New tokens saved...")
            print(
                f"Comparing access_token: Expected 'new_aws_sso_access_token', Got '{saved_data['access_token']}'"
            )
            assert saved_data["access_token"] == "new_aws_sso_access_token"

            print(
                f"Comparing refresh_token: Expected 'new_aws_sso_refresh_token', Got '{saved_data['refresh_token']}'"
            )
            assert saved_data["refresh_token"] == "new_aws_sso_refresh_token"
            ...
            print(
                f"Comparing futureField: Expected 'future_value', Got '{saved_data.get('futureField')}'"
            )
...
class TestAPIRegionAutoDetectionSQLite:
    ...
    def test_api_region_from_profile_arn_success(self, temp_sqlite_db_with_profile_arn):
        ...
        print(
            f"Comparing _detected_api_region: Expected 'eu-central-1', Got '{manager._detected_api_region}'"
        )
    ...
    def test_api_region_env_var_overrides_arn(
        self, temp_sqlite_db_with_profile_arn, monkeypatch
    ):
        """
        ...
        print(
            f"Setup: Creating KiroAuthManager with SQLite (ARN region=eu-central-1)..."
        )
        manager = KiroAuthManager(sqlite_db=temp_sqlite_db_with_profile_arn)
    ...
    def test_api_region_fallback_when_no_profile_key(
        self, temp_sqlite_db_with_empty_state_table
    ):
        """
        ...
        print("Verification: API hosts use SSO region as fallback...")
        print(
            f"Comparing _sso_region: Expected 'us-west-2', Got '{manager._sso_region}'"
        )
    ...
    def test_api_region_invalid_arn_format(self, temp_sqlite_db_with_invalid_arn):
        ...
        print("Verification: API hosts use SSO region as fallback...")
        print(
            f"Comparing _sso_region: Expected 'ap-southeast-1', Got '{manager._sso_region}'"
        )
    ...
    def test_api_region_malformed_json_in_state_table(
        self, temp_sqlite_db_with_malformed_state_json
    ):
        """
        ...
        print("Verification: API hosts use SSO region as fallback...")
        print(
            f"Comparing _sso_region: Expected 'ap-south-1', Got '{manager._sso_region}'"
        )
...
class TestAPIRegionAutoDetectionJSON:
    ...
    def test_api_region_from_json_region_field(self, temp_creds_file):
        ...
        print(
            f"Comparing _detected_api_region: Expected 'us-east-1', Got '{manager._detected_api_region}'"
        )
        assert manager._detected_api_region == "us-east-1"

        print(
            f"Comparing _sso_region: Expected 'us-east-1', Got '{manager._sso_region}'"
        )
...
class TestAPIRegionPriorityHierarchy:
    ...
    def test_auth_manager_api_region_parameter(self, temp_creds_file, monkeypatch):
        ...
        print(
            "Verification: But API hosts use api_region parameter (highest priority)..."
        )
        print(f"api_host: {manager._api_host}")
        assert "eu-central-1" in manager._api_host
    ...
    def test_auth_manager_api_region_priority_hierarchy(
        self, temp_sqlite_db_with_profile_arn, monkeypatch
    ):
        """
        ...
        manager2 = KiroAuthManager(
            sqlite_db=temp_sqlite_db_with_profile_arn, region="ap-south-1"
        )
