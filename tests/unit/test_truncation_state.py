...
class TestToolTruncation:
    ...
    def test_save_and_retrieve_tool_truncation(self):
        ...
        assert isinstance(info, ToolTruncationInfo), (
            "Should return ToolTruncationInfo instance"
        )
        assert info.tool_call_id == tool_call_id, (
            f"Expected tool_call_id={tool_call_id}, got {info.tool_call_id}"
        )
        assert info.tool_name == tool_name, (
            f"Expected tool_name={tool_name}, got {info.tool_name}"
        )
    ...
    def test_multiple_tool_truncations(self):
        ...
        assert stats["tool_truncations"] == 0, (
            "Cache should be empty after all retrievals"
        )
...
class TestContentTruncation:
    ...
    def test_save_and_retrieve_content_truncation(self):
        ...
        assert len(content_hash) == 16, (
            f"Hash should be 16 chars, got {len(content_hash)}"
        )
        ...
        assert info is not None, "First retrieval should return info"
        assert isinstance(info, ContentTruncationInfo), (
            "Should return ContentTruncationInfo instance"
        )
    ...
    def test_content_hash_uses_first_500_chars(self):
        ...
        # Assert
        assert hash1 == hash2, (
            "Content with same first 500 chars should produce same hash"
        )
...
class TestThreadSafety:
    ...
    def test_concurrent_tool_truncation_saves(self):
        ...
        assert len(set(tool_ids)) == num_threads, (
            "All tool_ids should be unique (no cross-contamination)"
        )
