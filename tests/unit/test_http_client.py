...
class TestKiroHttpClientRequestWithRetry:
    ...
    async def test_max_retries_exceeded_raises_502(self, mock_auth_manager_for_http):
        ...
        print(
            f"Verification: HTTPException with code 504 (timeout errors now return 504)..."
        )
...
class TestKiroHttpClientStreamingTimeout:
    ...
    async def test_connect_timeout_logged_correctly(self, mock_auth_manager_for_http):
        ...
        print(
            "Verification: logger.warning called with user-friendly timeout message..."
        )
    ...
    async def test_streaming_timeout_returns_504_with_error_type(
        ...
        assert exc_info.value.status_code == 504
        print(
            f"Comparing detail: Expected timeout message with troubleshooting in '{exc_info.value.detail}'"
        )
        assert "timeout" in exc_info.value.detail.lower()
        assert (
            "Troubleshooting" in exc_info.value.detail
            or "Technical details" in exc_info.value.detail
        )
    ...
    async def test_non_streaming_timeout_returns_502(self, mock_auth_manager_for_http):
        ...
        print(
            "Verification: HTTPException with code 504 (timeouts now consistently return 504)..."
        )
        assert exc_info.value.status_code == 504
...
class TestKiroHttpClientConnectionCloseHeader:
    ...
    @pytest.mark.asyncio
    async def test_streaming_request_includes_connection_close_header(
        self, mock_auth_manager_for_http
    ):
        """
        ...
        print(
            f"Comparing Connection: Expected 'close', Got '{captured_headers['Connection']}'"
        )
        assert captured_headers["Connection"] == "close"
        assert response.status_code == 200

    @pytest.mark.asyncio
    async def test_non_streaming_request_does_not_include_connection_close_header(
        self, mock_auth_manager_for_http
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_streaming_connection_close_preserves_other_headers(
        self, mock_auth_manager_for_http
    ):
        """
        ...
        with patch.object(http_client, "_get_client", return_value=mock_client):
            with patch(
                "kiro.http_client.get_kiro_headers",
                return_value=original_headers.copy(),
            ):
                response = await http_client.request_with_retry(
                    "POST",
                    "https://api.example.com/test",
...
class TestKiroHttpClientRequestParameters:
    ...
    async def test_request_with_retry_get_with_params(self, mock_auth_manager_for_http):
        ...
        assert "params" in captured_kwargs, (
            f"params not found in kwargs: {captured_kwargs}"
        )
        ...
        assert "json" not in captured_kwargs, (
            f"json should not be present for GET: {captured_kwargs}"
        )
        assert response.status_code == 200

    @pytest.mark.asyncio
    async def test_request_with_retry_post_without_json(
        self, mock_auth_manager_for_http
    ):
        """
        ...
        assert "json" not in captured_kwargs, (
            f"json should not be present when None: {captured_kwargs}"
        )
        assert "params" not in captured_kwargs, (
            f"params should not be present when None: {captured_kwargs}"
        )
        assert response.status_code == 200
