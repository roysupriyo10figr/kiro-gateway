...
class TestLifespanLegacyFallback:
    ...
    @pytest.mark.asyncio
    async def test_lifespan_legacy_mode_recreate_credentials(
        ...
        monkeypatch.setattr(
            "main.PROFILE_ARN", "arn:aws:codewhisperer:us-east-1:123456789:profile/test"
        )
    ...
    @pytest.mark.asyncio
    async def test_lifespan_account_system_one_time_migration(
        self, tmp_path, monkeypatch
    ):
        """
    ...
    async def test_lifespan_migration_add_env_overrides(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv(
            "PROFILE_ARN", "arn:aws:codewhisperer:eu-central-1:123456789:profile/test"
        )
        ...
        assert (
            creds[0]["profile_arn"]
            == "arn:aws:codewhisperer:eu-central-1:123456789:profile/test"
        )
...
class TestLifespanAccountManagerInit:
    ...
    @pytest.mark.asyncio
    async def test_lifespan_initialize_first_working_account(
        self, tmp_path, monkeypatch
    ):
        """
    ...
    async def test_lifespan_exit_if_all_failed(self, tmp_path, monkeypatch):
        ...
        with patch("main.AccountManager", return_value=mock_manager):
            with patch("main.httpx.AsyncClient") as mock_client_class:
                ...
                # Assert: RuntimeError is raised
                with pytest.raises(
                    RuntimeError, match="Failed to initialize any account"
                ):
                    async with lifespan(app):
                        pass
