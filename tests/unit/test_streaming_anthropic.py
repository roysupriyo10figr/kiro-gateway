...
class TestCollectAnthropicResponse:
    ...
    async def test_includes_usage_info(
        ...
        with patch(
            ...
            with patch(
                "kiro.streaming_anthropic.estimate_request_tokens",
...
class TestStreamingAnthropicContextUsage:
    ...
    @pytest.mark.asyncio
    async def test_uses_tools_and_system_for_input_tokens(
        self, mock_response, mock_model_cache, mock_auth_manager
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_context_usage_zero_keeps_fallback_estimate(
        self, mock_response, mock_model_cache, mock_auth_manager
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_non_streaming_passes_upstream_cache_usage_fields(
        self, mock_response, mock_model_cache, mock_auth_manager
    ):
        """
        ...
        with patch(
            ...
            with patch(
                "kiro.streaming_anthropic.generate_message_id", return_value="msg_test"
            ):
...
class TestStreamingAnthropicTruncationDetection:
    ...
    @pytest.mark.asyncio
    async def test_stop_reason_is_max_tokens_when_truncated(
        self, mock_response, mock_model_cache, mock_auth_manager
    ):
        """
        ...
        async def mock_parse_kiro_stream(*args, **kwargs):
            yield KiroEvent(
                type="content", content="This response was cut off mid-sentence because"
            )
        ...
        assert len(message_delta_events) >= 1
        print(
            f"Comparing stop_reason: Expected 'max_tokens', Got event: {message_delta_events[0]}"
        )
    ...
    async def test_stop_reason_is_tool_use_even_without_completion_signals(
        ...
        assert len(message_delta_events) >= 1
        print(
            f"Comparing stop_reason: Expected 'tool_use', Got event: {message_delta_events[0]}"
        )
        assert "tool_use" in message_delta_events[0]
    ...
    async def test_stop_reason_is_end_turn_with_completion_signals(
        ...
        assert len(message_delta_events) >= 1
        print(
            f"Comparing stop_reason: Expected 'end_turn', Got event: {message_delta_events[0]}"
        )
        assert "end_turn" in message_delta_events[0]
