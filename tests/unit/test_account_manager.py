...
class TestAccountManagerLoadCredentials:
    ...
    async def test_load_credentials_json_type(self, tmp_path):
        ...
        manager = AccountManager(
            credentials_file=str(creds_file), state_file=str(tmp_path / "state.json")
        )
    ...
    async def test_load_credentials_refresh_token_type(self, tmp_path):
        ...
        state_file.write_text(
            json.dumps(
                {"current_account_index": 0, "model_to_accounts": {}, "accounts": {}}
            )
        )

        manager = AccountManager(
            credentials_file=str(creds_file), state_file=str(state_file)
        )
...
class TestAccountManagerLoadState:
    ...
    async def test_load_state_success(self, tmp_path, sample_state_with_data):
        ...
        creds_file.write_text(
            json.dumps([{"type": "json", "path": str(test_json), "enabled": True}])
        )
    ...
    async def test_load_state_restore_current_account_index(self, tmp_path):
        ...
        manager = AccountManager(
            credentials_file=str(tmp_path / "creds.json"), state_file=str(state_file)
        )
...
class TestAccountManagerInitializeAccount:
    ...
    @pytest.mark.asyncio
    async def test_initialize_account_json_success(
...
class TestAccountManagerGetNextAccount:
    ...
    @pytest.mark.asyncio
    async def test_get_next_account_single_bypass_circuit_breaker(
        self, tmp_path, mock_list_models_response
    ):
        """
...
class TestAccountManagerReportSuccess:
    ...
    @pytest.mark.asyncio
    async def test_report_success_reset_failures(
        self, tmp_path, mock_list_models_response
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_report_success_update_stats(
        self, tmp_path, mock_list_models_response
    ):
        """
        ...
        print(
            f"Stats: total={stats.total_requests}, successful={stats.successful_requests}"
        )
...
class TestAccountManagerReportFailure:
    ...
    @pytest.mark.asyncio
    async def test_report_failure_recoverable_increment_failures(
        self, tmp_path, mock_list_models_response
    ):
        """
        ...
        await manager.report_failure(
            account_id, "claude-opus-4.5", ErrorType.RECOVERABLE, 429, None
        )
    ...
    @pytest.mark.asyncio
    async def test_report_failure_fatal_no_increment(
        self, tmp_path, mock_list_models_response
    ):
        """
...
class TestAccountManagerGetAllAvailableModels:
    ...
    @pytest.mark.asyncio
    async def test_get_all_available_models_collect_from_all(
        self, tmp_path, mock_list_models_response
    ):
        """
