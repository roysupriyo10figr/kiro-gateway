...
def test_vpn_proxy_environment_setup(
    ...
    if expected_http_proxy:
        ...
        print(
            f"HTTP_PROXY: Expected '{expected_http_proxy}', Got '{actual_http_proxy}'"
        )
        assert actual_http_proxy == expected_http_proxy, "HTTP_PROXY mismatch!"

        actual_https_proxy = os.environ.get("HTTPS_PROXY")
        print(
            f"HTTPS_PROXY: Expected '{expected_https_proxy}', Got '{actual_https_proxy}'"
        )
...
def test_proxy_scheme_normalization():
    ...
    for input_url, expected_url in test_cases:
        ...
        proxy_url_with_scheme = (
            input_url if "://" in input_url else f"http://{input_url}"
        )

        print(f"Result: '{proxy_url_with_scheme}'")
        print(f"Expected: '{expected_url}'")
        assert proxy_url_with_scheme == expected_url, (
            f"Normalization failed for '{input_url}'"
        )
...
def test_proxy_with_special_characters():
    ...
    for input_url, expected_url in test_cases:
        ...
        print(f"Expected: '{expected_url}'")
        assert proxy_url_with_scheme == expected_url, (
            f"Special chars handling failed for '{input_url}'"
        )
