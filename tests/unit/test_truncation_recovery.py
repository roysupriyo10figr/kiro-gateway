...
class TestToolTruncationMessage:
    ...
    def test_generate_truncation_tool_result_format(self):
        ...
        result = generate_truncation_tool_result(
            tool_name, tool_use_id, truncation_info
        )
        ...
        assert result["tool_use_id"] == tool_use_id, (
            f"tool_use_id should be {tool_use_id}"
        )
        ...
        assert "if" in content.lower() or "likely" in content.lower(), (
    ...
    def test_generate_truncation_tool_result_different_tools(self):
        ...
        for tool_name, tool_id in tools:
            ...
            assert result["tool_use_id"] == tool_id, (
                f"Should preserve tool_id for {tool_name}"
            )
            assert "[API Limitation]" in result["content"], (
                f"Should have marker for {tool_name}"
            )
    ...
    def test_generate_truncation_tool_result_no_specific_instructions(self):
        ...
        for phrase in forbidden_phrases:
            assert phrase not in content, (
                f"Should NOT contain specific instruction: '{phrase}'"
            )

        # Assert - Should contain general guidance
        assert "adapt" in content or "consider" in content, (
            "Should suggest general adaptation"
        )
...
class TestContentTruncationMessage:
    ...
    def test_generate_truncation_user_message_format(self):
        ...
        assert "output size" in message.lower() or "size limit" in message.lower(), (
            "Should mention size limits"
        )

        # Assert - Not model's fault
        assert (
            "not an error on your part" in message.lower()
            or "not your fault" in message.lower()
    ...
    def test_generate_truncation_user_message_no_micro_steps(self):
        ...
        for phrase in forbidden_phrases:
            assert phrase not in content, (
                f"Should NOT contain micro-step trigger: '{phrase}'"
            )
    ...
    def test_generate_truncation_user_message_consistency(self):
        ...
        assert message1 == message2 == message3, (
            "Should generate same message each time (deterministic)"
        )
...
class TestMessageIntegration:
    ...
    def test_user_message_can_be_inserted_after_assistant(self):
        ...
        assert "[System Notice]" in conversation[-1]["content"], (
            "Should contain system notice"
        )
