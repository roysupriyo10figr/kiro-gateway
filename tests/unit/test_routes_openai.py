...
class TestTruncationRecoveryMessageModification:
    ...
    def test_modifies_tool_result_with_truncation_notice(self):
        ...
        save_tool_truncation(
            tool_call_id, "write_to_file", {"size_bytes": 5000, "reason": "test"}
        )
    ...
    def test_no_modification_when_no_truncation(self):
        ...
        messages = [
            ChatMessage(
                role="tool", tool_call_id="tooluse_nonexistent", content="Success"
            )
        ]
    ...
    def test_pydantic_immutability_new_object_created(self):
        ...
        save_tool_truncation(
            tool_call_id, "tool", {"size_bytes": 1000, "reason": "test truncation"}
        )

        original_msg = ChatMessage(
            role="tool", tool_call_id=tool_call_id, content="original"
        )
        ...
        if (
            original_msg.role == "tool"
            and original_msg.tool_call_id
...
class TestTruncationRecoveryEdgeCases:
    ...
    def test_orphaned_tool_result_no_crash(self):
        ...
        messages = [
            ChatMessage(
                role="tool", tool_call_id="tooluse_nonexistent_orphan", content="Result"
            )
        ]
    ...
    def test_empty_tool_result_content(self):
        ...
        save_tool_truncation(
            tool_call_id, "tool", {"size_bytes": 1000, "reason": "test"}
        )

        messages = [ChatMessage(role="tool", tool_call_id=tool_call_id, content="")]
...
class TestContentTruncationRecovery:
    ...
    def test_adds_synthetic_user_message_after_truncated_assistant(self):
        ...
        messages = [ChatMessage(role="assistant", content=truncated_content)]
    ...
    def test_auto_injection_logic_openai(self):
        ...
        if WEB_SEARCH_ENABLED:
            has_ws = any(
                ...
                and getattr(getattr(tool, "function", None), "name", None)
                == "web_search"
                for tool in tools
            )
...
class TestChatCompletionsFailoverLoop:
    ...
    async def test_chat_completions_failover_success_first_account(self):
        ...
        mock_manager.report_success.assert_called_once_with(
            "/home/user/account1.json", "claude-opus-4.5"
        )
