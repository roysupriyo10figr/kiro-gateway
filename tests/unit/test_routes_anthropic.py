...
class TestTruncationRecoveryMessageModification:
    ...
    def test_modifies_tool_result_dict_with_truncation_notice(self):
        ...
        save_tool_truncation(
            tool_use_id, "write_to_file", {"size_bytes": 5000, "reason": "test"}
        )
            ...
            if msg.role == "user" and msg.content and isinstance(msg.content, list):
                ...
                for block in msg.content:
                    ...
                    if (
                        ...
                        and block_tool_use_id
                        and should_inject_recovery()
                    ):
        ...
        messages = [
            AnthropicMessage(
                ...
                content=[
                    {
                        "type": "tool_result",
                        "tool_use_id": "tooluse_nonexistent",
                        "content": "Success",
                    }
                ],
            )
    ...
    def test_pydantic_immutability_new_object_created(self):
        ...
        save_tool_truncation(
            tool_use_id, "tool", {"size_bytes": 1000, "reason": "test truncation"}
        )
        ...
        if (
            original_msg.role == "user"
            and original_msg.content
            and isinstance(original_msg.content, list)
        ):
            modified_content_blocks = []
            has_modifications = False
            ...
            if has_modifications:
                modified_msg = original_msg.model_copy(
                    update={"content": modified_content_blocks}
                )

        print("Checking: Original message unchanged...")
        assert (
            self._get_block_value(original_msg.content[0], "content")
            == original_content
        )
...
class TestContentTruncationRecovery:
    ...
    def test_adds_synthetic_user_message_after_truncated_assistant(self):
        ...
        messages = [
            AnthropicMessage(
                role="assistant",
                content=[{"type": "text", "text": truncated_content_text}],
            )
        ]
                    ...
                    if truncation_info:
                        ...
                        synthetic_user_msg = AnthropicMessage(
                            ...
                            content=[
                                {
                                    "type": "text",
                                    "text": generate_truncation_user_message(),
                                }
                            ],
                        )
...
class TestWebSearchAutoInjection:
    ...
    def test_no_duplicate_injection_logic(self):
        ...
        if WEB_SEARCH_ENABLED:
            has_ws = any(
                getattr(tool, "name", "") == "web_search" for tool in existing_tools
            )
...
class TestWebSearchNativeDetection:
    ...
    def test_native_tool_type_detection(self):
        ...
        tools = [
            AnthropicTool(type="web_search_20250305", name="web_search", max_uses=8)
        ]
...
class TestMessagesFailoverLoop:
    ...
    async def test_messages_failover_get_next_account(self):
        ...
        result = await mock_manager.get_next_account(
            model, exclude_accounts=exclude_accounts
        )

        print("Checking: get_next_account was called with correct parameters...")
        mock_manager.get_next_account.assert_called_once_with(
            model, exclude_accounts=exclude_accounts
        )
    ...
    async def test_messages_failover_all_unavailable(self):
        ...
        account = await mock_manager.get_next_account(
            "claude-opus-4.5", exclude_accounts=set()
        )
...
class TestCountTokensEndpoint:
    ...
    def test_count_tokens_no_max_tokens_required(
        self, test_client, valid_proxy_api_key
    ):
        """
