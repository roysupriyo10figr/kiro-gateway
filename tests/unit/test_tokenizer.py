...
class TestCountTokens:
    ...
    def test_claude_correction_applied_by_default(self):
        ...
        assert with_correction > without_correction, (
            "With correction should have more tokens"
        )
...
class TestCountToolsTokens:
    ...
    def test_openai_flat_tool_format(self):
        ...
        assert (
            result > 4
        )  # Must exceed base service overhead, proving name/description/schema are counted
    ...
    def test_anthropic_flat_and_openai_function_are_close(self):
        ...
        anthropic_tokens = count_tools_tokens(
            anthropic_tools, apply_claude_correction=False
        )

        assert openai_tokens > 4
        assert anthropic_tokens > 4
        diff_ratio = abs(openai_tokens - anthropic_tokens) / max(
            openai_tokens, anthropic_tokens
        )
...
class TestCountSystemTokens:
    ...
    def test_plain_string(self):
        ...
        result = count_system_tokens(
            "You are a helpful assistant.", apply_claude_correction=False
        )
        assert result > 0
...
class TestEstimateRequestTokens:
    ...
    def test_anthropic_system_blocks(self):
        ...
        system_prompt = [
            ...
            {
                "type": "text",
                "text": "Follow tools strictly",
                "cache_control": {"type": "ephemeral"},
            },
        ]

        result = estimate_request_tokens(
            messages, system_prompt=system_prompt, apply_claude_correction=False
        )
        print(f"Result: {result}")
