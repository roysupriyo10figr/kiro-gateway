...
class TestNativeReasoningFlow:
    """Verify native reasoning from request conversion through response encoding."""
    ...
    async def test_native_events_preserve_text_and_signature(
        ...
        cache = MagicMock()
        cache.get_max_input_tokens.return_value = 1000000
        auth = MagicMock()
        if api == "anthropic":
            if stream:
                events = [
                ...
                assert (
                    "".join(delta.get("thinking", "") for delta in deltas)
                    == "reasonreason"
                )
                assert (
                    "".join(delta.get("signature", "") for delta in deltas) == "signed"
                )
            else:
            ...
            events = [
                ...
                async for chunk in stream_kiro_to_openai(
                    AsyncMock(), response, "claude-opus-5", cache, auth
                )
            ]
            decoded = [
                json.loads(event[6:])
            ...
            assert (
                "".join(delta.get("reasoning_signature", "") for delta in deltas)
                == "signed"
            )
        else:
            ...
            assert message["reasoning_content"] == "reasonreason"
            assert message["reasoning_signature"] == "signed"
