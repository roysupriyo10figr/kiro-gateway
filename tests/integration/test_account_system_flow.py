...
class TestAccountSystemFullFlow:
    ...
    async def test_full_failover_flow_two_accounts(
        ...
        await manager.report_failure(
            account1_id, "claude-opus-4.5", ErrorType.RECOVERABLE, 429, None
        )
        ...
        print(
            f"Account 2 succeeded: failures={manager._accounts[account2_id].failures}"
        )

        # 4. Verify sticky behavior - should prefer account2 now
        next_account_again = await manager.get_next_account("claude-opus-4.5")
        print(
            f"Next account (sticky): {next_account_again.id if next_account_again else None}"
        )
    ...
    async def test_circuit_breaker_blocks_broken_account(
        self, tmp_path, temp_account_credentials_files, mock_time
    ):
        """
    ...
    async def test_half_open_recovery_after_timeout(
        self, tmp_path, temp_account_credentials_files
    ):
        """
    ...
    async def test_state_persistence_across_restarts(
        ...
        print(
            f"Saved state: failures={account.failures}, stats={account.stats.total_requests}"
        )
        ...
        print(
            f"Restored state: failures={account2.failures}, stats={account2.stats.total_requests}"
        )
    ...
    async def test_ttl_refresh_on_usage(
        ...
        print(
            f"Cache age: {time.time() - account.models_cached_at}s (TTL: {ACCOUNT_CACHE_TTL}s)"
        )
