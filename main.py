...
async def lifespan(app: FastAPI):
    ...
    has_refresh_token = bool(REFRESH_TOKEN)
    has_creds_file = (
        bool(KIRO_CREDS_FILE) and Path(KIRO_CREDS_FILE).expanduser().exists()
    )
    ...
    if ACCOUNT_SYSTEM:
        ...
        if not creds_path.exists():
            if has_refresh_token or has_creds_file or has_cli_db:
                logger.info(
                    "credentials.json not found, creating from .env (one-time migration)"
                )
                credentials = []
    ...
    app.state.account_manager = AccountManager(
        credentials_file=ACCOUNTS_CONFIG_FILE, state_file=ACCOUNTS_STATE_FILE
    )
