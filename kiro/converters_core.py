...
class ThinkingConfig:
    ...
    Attributes:
        ...
        native_fields: Native Kiro model controls; bypass prompt injection when set.
...
def inject_thinking_tags(content: str, thinking_config: ThinkingConfig) -> str:
    ...
    if thinking_config.native_fields is not None or not thinking_config.enabled:
        logger.debug("Skipping fake thinking injection for native or disabled thinking")
        return content
...
def ensure_first_message_is_user(
    ...
    if messages[0].role != "user":
        ...
        synthetic_user = UnifiedMessage(role="user", content="(empty placeholder)")
