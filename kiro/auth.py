...
class KiroAuthManager:
    ...
    def __init__(
        ...
        self._client_id_hash: Optional[str] = (
            None  # clientIdHash from Enterprise Kiro IDE
        )
        ...
        if api_region:
            ...
            logger.info(
                f"API region: {final_api_region} (from KIRO_API_REGION env var)"
            )
            ...
            logger.info(
                f"API region: {final_api_region} (auto-detected from credentials)"
            )
        elif self._sso_region:
            # Fallback to SSO region
            final_api_region = self._sso_region
            logger.info(
                f"API region: {final_api_region} (using SSO region as fallback)"
            )
        else:
                ...
                if profile_row:
                    ...
                    if arn:
                        if not self._profile_arn:
                            ...
                            logger.debug(
                                f"Profile ARN from state table: {self._profile_arn}"
                            )
                        ...
                        if len(parts) >= 4 and parts[3]:
                            ...
                            if re.match(r"^[a-z]+-[a-z]+-\d+$", parts[3]):
                                ...
                                logger.info(
                                    f"API region auto-detected from profile ARN: {parts[3]}"
                                )
                            else:
                                logger.debug(
                                    f"Invalid region format in ARN: {parts[3]}"
                                )
        ...
        try:
            ...
            if self._sqlite_token_key:
                if self._try_save_to_key(cursor, self._sqlite_token_key):
                    ...
                    conn.close()
                    logger.debug(
                        f"Credentials saved to SQLite key: {self._sqlite_token_key} (merged)"
                    )
                    return
                else:
                    logger.warning(
                        f"Failed to save to primary key: {self._sqlite_token_key}, trying fallback"
                    )
            ...
            for key in SQLITE_TOKEN_KEYS:
                if self._try_save_to_key(cursor, key):
                    ...
                    conn.close()
                    logger.debug(
                        f"Credentials saved to SQLite key: {key} (fallback, merged)"
                    )
                    return
            ...
            conn.close()
            logger.warning(
                f"Failed to save credentials to SQLite: no matching keys found"
            )
    ...
    def _try_save_to_key(self, cursor: sqlite3.Cursor, key: str) -> bool:
        ...
        try:
            ...
            cursor.execute(
                "UPDATE auth_kv SET value = ? WHERE key = ?", (token_json, key)
            )
