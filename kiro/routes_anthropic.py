...
async def messages(
    ...
    if WEB_SEARCH_ENABLED:
        ...
        has_ws = any(
            getattr(tool, "name", "") == "web_search" for tool in request_data.tools
        )
    ...
    if request_data.tools:
        for tool in request_data.tools:
            ...
            if tool_type and tool_type.startswith("web_search"):
                ...
                logger.info(
                    "Detected native Anthropic web_search (Path A), routing to MCP API"
                )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if request_data.stream:
                        ...
                        async def stream_wrapper():
                            ...
                            try:
                                ...
                                async for (
                                    chunk
                                ) in stream_with_first_token_retry_anthropic(
                        ...
                        await http_client.close()
                        logger.info(
                            f"HTTP 200 - POST /v1/messages (non-streaming) - completed"
                        )
                    ...
                    if error_type == ErrorType.FATAL:
                        ...
                        logger.warning(
                            f"HTTP {response.status_code} - POST /v1/messages - {last_error_message[:100]}"
                        )
