            ...
            if event.type == "content" and event.content:
                ...
                signature_chunk = {
                    ...
                    "choices": [
                        {
                            "index": 0,
                            "delta": {"reasoning_signature": event.thinking_signature},
                            "finish_reason": None,
                        }
                    ],
                }
                ...
                tool_name = ""
                if tool:
                    tool_name = (tool.get("function") or {}).get(
                        "name", ""
                    ) or tool.get("name", "")
                ...
                if tool_name == "web_search":
                    ...
                    if not query:
                        logger.warning(
                            "web_search called without query, skipping MCP call"
                        )
...
async def collect_stream_response(
    ...
    async for chunk_str in stream_kiro_to_openai(
        ...
        try:
            ...
            if "reasoning_signature" in delta:
                full_reasoning_signature += delta["reasoning_signature"]
            if "tool_calls" in delta:
                tool_calls.extend(delta["tool_calls"])

            # Extract finish_reason from chunk (streaming already calculated it correctly)
            finish_reason_from_chunk = chunk_data.get("choices", [{}])[0].get(
                "finish_reason"
            )
    ...
    if full_reasoning_signature:
        message["reasoning_signature"] = full_reasoning_signature
    if tool_calls:
