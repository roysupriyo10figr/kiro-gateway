...
async def get_models(request: Request):
    ...
    if request.app.state.account_system:
        ...
        available_model_ids = (
            request.app.state.account_manager.get_all_available_models()
        )
    else:
    ...
    if request.app.state.account_system:
        ...
        for attempt in range(MAX_ATTEMPTS):
            ...
            try:
                ...
                logger.error(
                    f"HTTP {e.status_code} - POST /v1/chat/completions - {e.detail}"
                )
                if debug_logger:
        ...
        if len(all_accounts) == 1:
            ...
            raise HTTPException(
                status_code=last_error_status, detail=last_error_message
            )
        else:
            # Multiple accounts - generic error with context
