...
class KiroEvent:
    ...
    Attributes:
        ...
        thinking_signature: Native upstream signature (for signature events)
        native_thinking: Whether reasoning came from a native event, not prompt tags
...
class StreamResult:
    ...
    Attributes:
        ...
        thinking_signature: Accumulated native upstream signature
...
async def _process_chunk(
    ...
    for event in events:
        if event["type"] == "thinking":
            yield KiroEvent(
                type="thinking", thinking_content=event["data"], native_thinking=True
            )
