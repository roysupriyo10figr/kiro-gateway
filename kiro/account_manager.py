...
class AccountManager:
    ...
    async def load_credentials(self) -> None:
        ...
        for entry in self._credentials_config:
            ...
            if cred_type in ("json", "sqlite") and not path:
                logger.warning(
                    f"Invalid credential entry (type={cred_type} requires path): {entry}"
                )
                continue
            ...
            if cred_type == "refresh_token" and not entry.get("refresh_token"):
                logger.warning(
                    f"Invalid credential entry (type=refresh_token requires refresh_token field): {entry}"
                )
                continue
                ...
                for file_path in expanded_path.iterdir():
                    ...
                    if cred_type == "json":
                        try:
                            ...
                            logger.warning(
                                f"Invalid JSON credentials file {file_path.name}: {e}"
                            )
                    ...
                    # Try SQLite validation
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Invalid SQLite database file {file_path.name}: {e}"
                            )
                    ...
                    if is_valid:
                        ...
                        logger.warning(
                            f"Skipping invalid credentials file: {file_path.name}"
                        )
    ...
    async def load_state(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Loaded state: {len(self._model_to_accounts)} model mappings, {len(self._accounts)} accounts"
            )
    ...
    async def _initialize_account(self, account_id: str) -> bool:
        ...
        try:
            ...
            if _is_runtime_endpoint(auth_manager):
                ...
                logger.debug(
                    f"Account {account_id}: Using static model list for runtime.kiro.dev endpoint"
                )
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to fetch models for {account_id} after retries: {e}"
                    )
                    logger.warning(
            ...
            logger.info(
                f"Initialized account: {account_id} ({len(available_models)} models)"
            )
            self._dirty = True
            return True
    ...
    async def _refresh_account_models(self, account_id: str) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Failed to refresh models for {account_id} after retries: {e}"
            )
    ...
    async def get_next_account(
        self, model: str, exclude_accounts: Optional[set] = None
    ) -> Optional[Account]:
        """
                ...
                if account.models_cached_at > 0:
                    ...
                    if age > ACCOUNT_CACHE_TTL:
                        try:
                            ...
                            logger.warning(
                                f"Failed to refresh models for {account_id}: {e}"
                            )
    ...
    async def report_success(self, account_id: str, model: str) -> None:
        ...
        async with self._lock:
            ...
            if normalized_model not in self._model_to_accounts:
                ...
                logger.debug(
                    f"Dynamic learning: discovered new model '{normalized_model}'"
                )
            if account_id not in self._model_to_accounts[normalized_model].accounts:
                ...
                logger.debug(
                    f"Dynamic learning: model '{normalized_model}' works on account {account_id}"
                )
                self._dirty = True
